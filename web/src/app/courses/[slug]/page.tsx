...
export default async function CourseDetailsPage({
  params
}: CourseDetailsPageProps): Promise<ReactElement> {
              ...
              <Card key={mod.id} className="space-y-3">
                ...
                <ul className="space-y-3">
                  {mod.lessons.map((lesson) => {
                    const canWatch = canAccessLessonContent({
                      ...
                      >
                        ...
                        {lesson.duration > 0 ? (
                          <p className="mt-1 text-xs text-neutral-500">
                            {format("courses.minutesShort", {
                              n: String(Math.round(lesson.duration / 60))
                            })}
                          </p>
                        ) : null}
                        {canWatch ? (
